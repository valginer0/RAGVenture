...
class OutputFormatter:
    def __init__(self):
        ...
        self.nested_ops: Set[str] = {'initialize_embeddings', 'create_and_split_document'}
        ...
        for category, operations in categories.items():
            ...
            for op in operations:
                if op in self.timings:
                    ...
                    if op not in self.nested_ops:  # Only add to total if not a nested operation
                        category_time += duration
                        total_time += duration
                    print(f"  • {op}: {duration:.2f}s")
            
            # Add nested operations at the end of their category
            if category == "Data Loading" and 'create_and_split_document' in self.timings:
                print(f"  • create_and_split_document: {self.timings['create_and_split_document']:.2f}s (combined)")
            elif category == "Embedding Process" and 'initialize_embeddings' in self.timings:
                print(f"  • initialize_embeddings: {self.timings['initialize_embeddings']:.2f}s (combined)")
    ...
    def print_startup_ideas(ideas: str):
        ...
        ideas = ideas.replace("Here are the most relevant startup ideas from YC companies:\n", "")
