...
def test_convert_csv_to_json(tmp_path):
    ...
    # Check returned data
    assert len(startups) == 4  # Should only include entries with both name and description
    ...
    weird = next(s for s in startups if "description has multiple lines" in s["description"])
